...
(function(window, document)
{
	...
	Shape_Type.prototype.convertShape = function (visioDocument,
		...
		if (layerColor) {
			...
			// See bug https://bugzilla.univaultoffice.com/show_bug.cgi?id=76044
		...
		if (isShadowVisible) {
			...
			function getShadows(shape, pageInfo, visioDocument) {
				...
				//  in https://bugzilla.univaultoffice.com/show_bug.cgi?id=75884
		...
		function getTextCShape(theme, shape, cShape, lineUniFill,
			...
			// for examples
			// https://api.univaultoffice.com/docbuilder/textdocumentapi just some related info
			let bWord = false;
			...
			// use https://api.univaultoffice.com/docbuilder/presentationapi/apishape api implementation code
	...
	Shape_Type.prototype.convertGroup = function (visioDocument, pageInfo,
		...
		if (this.type === AscVisio.SHAPE_TYPES_GROUP) {
			...
			if (cShapeOrCGroupShape) {
				...
				/**
				 * see bug for Del attribute handle: https://bugzilla.univaultoffice.com/show_bug.cgi?id=76050
